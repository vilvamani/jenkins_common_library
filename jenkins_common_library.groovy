...
def dockerize(configs) {
    stage('Build docker image') {
        dir(configs.branch_checkout_dir) {
            def customImage = docker.build(configs.dockerRepoURL + "/" + configs.imageName + ":" + configs.git_commit_id)

            sh "docker tag ${configs.dockerImage} + "/" + ${configs.imageName} +":" + ${configs.git_commit_id}"
